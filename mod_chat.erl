...
leave_chat(#jid{user = User, server = Server, resource = _R}, ChatId, ParticipantUsername) ->
	...
	ejabberd_odbc:sql_query(Server,
                                        [<<"UPDATE participants SET status='inactive' WHERE chat_id=(SELECT id FROM chat WHERE uuid='">>,ChatId,<<"') AND username='">>,ParticipantUsername,<<"'">>]),
...
on_user_send_packet(#jid{user = User, server = Server,
	...
	?INFO_MSG("ON_USER_SEND_MSG: ~p", [ToUser]),

	case Type == <<"chat">> of
		true ->
			ejabberd_odbc:sql_query(Server,
                                [<<"UPDATE participants SET status='active' WHERE chat_id=(SELECT id FROM chat WHERE uuid='">>,ToUser,<<"' AND (type='121' OR type='thought'))">> ]);
		_ ->
			[]
	end,
